...
private:
	...
	struct face_data_t
	{
		face_data_t(face_t* _face, const face_t* _orig_face, bool _flipped) : face(_face),
...
// ---------------------------------------------------------------------------- //
CMultiDomainCollector::~CMultiDomainCollector() = default;
		...
		carve::csg::CSG::Hooks& hooks)
{
	...
	for (size_t i = 0; i < new_faces.size(); ++i)
	{
		...
		else if ((face_class == carve::csg::FACE_ON) || (face_class == carve::csg::FACE_ON_ORIENT_IN) || (face_class == carve::csg::FACE_ON_ORIENT_OUT))
		{
...
void CMultiDomainPreprocessor::GetMergedDomains(QGenTriangleMesh& merged, std::vector<std::array<int, 2>>& tags_out) const
{
	...
	// Clean up
	std::shared_ptr<int> cleanup(new int, [&](int*) {
		delete mesh;
		delete tags;
	});
...
bool CMultiDomainPreprocessor::Update()
{
			...
			for (size_t jj = ii + 1; jj < nDomains; ++jj)
			{
				...
				/*if ( intersect )*/
				{
					...
					// Process domain pair (ii,jj)
					{
						...
						if (!out0 || !out1)
						{
							msg << "CMultiDomainPreprocessor::Update "
	...
	for (size_t i = 0; i < nDomains; ++i)
	{
		...
		if (!XCore::Modeling::ExtractFaceSubSet(*mesh, out, rule) || !XCore::Modeling::ExtractTagSubSet(*mesh, *out, tags, tags_out_ptr, rule))
		{
...
bool CMultiDomainPreprocessor::AssignPrioritiesBaseOnTopology()
{
	...
	// Functor(s)
	auto containedAABBs = [](const AABB& x, const AABB& y) {
		// Scope variables
		double zmin, zmax;
